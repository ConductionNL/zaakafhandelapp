...
Per-user preference read/write surface of the case-handling app. Covers retrieving
and persisting per-user key/value preferences scoped to the authenticated user
within the `pref_` namespace, with key sanitisation and empty-value clearing.
Reverse-specified from observed `PreferencesController` behavior.
