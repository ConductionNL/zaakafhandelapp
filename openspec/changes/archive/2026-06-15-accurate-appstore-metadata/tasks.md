...
- [x] 1.1. Replace `<summary>` with a complete one-line English pitch, e.g.
...
- [x] 1.2. Add `<summary lang="nl">` with the Dutch equivalent (e.g.
      "Zaakgericht werken voor Nextcloud — zaken, taken, klantcontacten en
      een ZGW-API, gebouwd op OpenRegister.").
- [x] 1.3. Replace the `<description>` CDATA body. Content:
...
- [x] 1.4. Add `<description lang="nl">` with the Dutch equivalent of 1.3.
- [x] 1.5. Delete the line "**System Cron is currently required for this app
      to work**" (no BackgroundJob classes exist under `lib/`).
- [x] 1.6. Replace the OpenCatalogi links: requirements link
      ...
      `<website>`/`<bugs>` host).
- [x] 1.7. Bump `<version>` patch (0.2.3 → 0.2.4) so the corrected metadata
      ships and caches bust.

## 2. Align README.md

- [x] 2.1. Change "**Message Audit Trail** — Full history of message edits
      ...
      wanted).
- [x] 2.2. Architecture diagram: remove the `C --> E[Elasticsearch]`,
      `A --> G[Nextcloud Activity]` and `H[Cron] -->|background jobs| C`
      nodes/edges; add/keep an OpenRegister node as the storage + search
      backend.
- [x] 2.3. Requirements table: remove the "Elasticsearch | optional — for
      full-text case search" row.
- [x] 2.4. Tech Stack table: change the Search row from "SQL ILIKE (default)
      + Elasticsearch (optional)" to search provided via OpenRegister.

## 3. Validation

- [x] 3.1. `xmllint --noout appinfo/info.xml` and validate against the
      Nextcloud `info.xsd` (the file's declared schema) — must pass with the
      new `lang="nl"` elements.
- [x] 3.2. Greps come back empty on the promise surface:
...
- [x] 3.3. Confirm no `BackgroundJob`/Cron classes were added meanwhile
      (`grep -rn 'BackgroundJob' lib/` empty) — otherwise REQ-META-002 needs
      re-evaluation instead of removal.
- [x] 3.4. Render the README mermaid diagram (GitHub/Codeberg preview) — no
      broken nodes.

## 4. Specs

- [x] 4.1. Sync `specs/appstore-metadata/spec.md` (4 ADDED requirements) to
      `openspec/specs/appstore-metadata/spec.md` on archive.

## 5. Out of scope (explicit)

- [x] 5.1. NO changes to `lib/`, `src/`, `appinfo/routes.php`. The 501 ZGW
