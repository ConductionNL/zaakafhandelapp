...
- [~] 1.1. Import `useObjectStore` from `@conduction/nextcloud-vue` — deferred to downstream cycle (handoff)
      in `src/store/store.js`.
- [~] 1.2. Re-export `useObjectStore` from `src/store/store.js` — deferred to downstream cycle (handoff)
      alongside the existing thirteen legacy `*Store` exports.
- [~] 1.3. Add `initializeStores()` async helper to — deferred to downstream cycle (handoff)
...
- [~] 1.4. Object types to register (eleven, all under register — deferred to downstream cycle (handoff)
...
- [~] 2.1. Import `initializeStores` from `./store/store.js` in — deferred to downstream cycle (handoff)
      `src/main.js`.
- [~] 2.2. Call `initializeStores()` after `Vue.use(PiniaVuePlugin)` — deferred to downstream cycle (handoff)
      and after `pinia` is imported, but before `new Vue(...).$mount`.
- [~] 2.3. Use the existing `tryLoadTranslations`-style — deferred to downstream cycle (handoff)
      fire-and-forget pattern — don't await the helper at boot.

## 3. Specs

- [~] 3.1. Add `specs/zaakafhandelapp-store/spec.md` with the — deferred to downstream cycle (handoff)
      requirements declared in the proposal.

## 4. Validation

- [~] 4.1. `npx eslint src/store/store.js src/main.js` — clean. — deferred to downstream cycle (handoff)
- [~] 4.2. `node tests/validate-manifest.js` — manifest still — deferred to downstream cycle (handoff)
      validates against the lib schema (no manifest change in
      this PR; should still pass as a sanity check).
- [~] 4.3. `npx webpack --mode production` — build succeeds. — deferred to downstream cycle (handoff)
- [~] 4.4. `npx jest --silent src/store` — existing store — deferred to downstream cycle (handoff)
...
- [~] 5.1. Cross-link from — deferred to downstream cycle (handoff)
