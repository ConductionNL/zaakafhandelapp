<!--
 ...
 The manifest-v2 migration replaced it with the declarative "ZaaktypeDetail"
 page (/zaaktypen/:id), which the library renderer draws through
 ...
 @visual exclude Dead file: superseded by the manifest-v2 "ZaaktypeDetail" page (/zaaktypen/:id); imported by no module in src/, so it is in no bundle and no e2e can mount it. The live page is covered by tests/e2e/spec-coverage/ui-detail-views.spec.ts. Correct fix is deletion.
-->
<script setup>
...
<template>
	<div class="detailContainer">
		<div id="app-content">
			<NcLoadingIcon v-if="!zaakTypeStore.zaakTypeItem && loading" :size="64" />

			<div v-if="zaakTypeStore.zaakTypeItem">
				<div class="head">
					<h1 class="h1">
...
export default {
	...
	methods: {
		...
		fetchData(id) {
			...
			zaakTypeStore.getZaakType(id, { setItem: true })
				.finally(() => {
					this.loading = false
				})
		},
