...
<template>
	<NcDialog
		...
		<div v-if="!isView" class="tabContainer">
			...
			<CnTabs
				card
				class="contactmomentTabsContainer"
				:aria-label="t('zaakafhandelapp', 'Open contact moments')">
				<CnTab
					v-for="i in tabs"
					:key="'dyn-tab-' + i"
					...
					<template #title>
						{{
							contactMomenten[i].klant
								? getInitials(contactMomenten[i].klant)
								: 'Leeg'
						}}
						<NcButton
					...
					<div>
						<div class="headerContainer">
							<div class="personInfoContainer">
								<NcNoteCard type="info" class="noteCard">
									<template #default>
										...
										<div v-else>Geen klant geselecteerd</div>
									</template>
								</NcNoteCard>
							</div>
							<div
								v-if="!contactMomenten[i].klant"
							...
							<div
								v-if="contactMomenten[i].klant && !isView"
								class="buttonsContainer">
								<div>
									<NcButton
										:disabled="loading"
						...
						<div class="form-group">
							<NcTextArea
								v-model="contactMomenten[i].notitie"
								label="Notitie"
								:disabled="loading"
								:loading="fetchLoading"
								placeholder="Notitie" />
						...
						<div class="tabContainer">
							<CnTabs
								...
								<CnTab
									...
									<div
										v-if="
											contactMomenten[i].klantContactmomenten
												?.length
										">
										<NcListItem
											v-for="(
												klantContactmoment, key
											) in contactMomenten[i]
									...
									<NcEmptyContent
										v-else
										icon="icon-folder"
										title="Geen contactmomenten gevonden">
										<template #description>
											Er zijn geen contactmomenten gevonden
											voor deze klant.
										</template>
									</NcEmptyContent>
								</CnTab>
								<CnTab
									...
									<div v-if="contactMomenten[i].zaken?.length">
										<NcListItem
											v-for="(zaak, key) in contactMomenten[i]
												.zaken"
											:key="key"
									...
									<NcEmptyContent
										v-else
										icon="icon-folder"
										title="Geen zaken gevonden">
										<template #description>
											Er zijn geen zaken gevonden voor deze
											klant.
										</template>
									</NcEmptyContent>
								</CnTab>
								<CnTab
									...
									<div v-if="contactMomenten[i].taken?.length">
										<NcListItem
											v-for="(taak, key) in contactMomenten[i]
												.taken"
											:key="key"
											:name="taak.title"
											:bold="false"
									...
									<NcEmptyContent
										...
										<template #description>
											Er zijn geen taken gevonden voor deze
											klant.
										</template>
									</NcEmptyContent>
								</CnTab>
								<CnTab
									...
									<div
										v-if="
											contactMomenten[selectedContactMoment]
												?.klant?.producten?.length
										">
										<NcListItem
											v-for="(product, key) in contactMomenten[
									...
									<NcEmptyContent
										...
										<template #description>
											Er zijn geen producten gevonden voor deze
											klant.
										</template>
									</NcEmptyContent>
								</CnTab>
							...
							<template #default>
								<div v-if="klant">
									...
									<div
										...
										<div>
											Locatie: {{ klant?.postcode ?? 'N/A' }}
											{{ klant?.straatnaam ?? 'N/A' }}
										</div>
									</div>
								</div>
								<div v-else>Geen klant geselecteerd</div>
					...
					<CnTabs
						...
						<CnTab
							...
							<div v-if="klantContactmomenten.length">
								<NcListItem
									...
									<template #icon>
										...
										<EmailOutline
											v-else-if="
												klantContactmoment.kanaal === 'email'
											"
											:size="44" />
										<FaceAgent
											v-else-if="
												klantContactmoment.kanaal === 'balie'
											"
											:size="44" />
										<MailboxOpenOutline
											v-else-if="
												klantContactmoment.kanaal === 'brief'
											"
											:size="44" />
							...
							<NcEmptyContent
								...
								title="Geen contactmomenten gevonden">
								<template #description>
									Er zijn geen contactmomenten gevonden voor deze
									klant.
								</template>
							</NcEmptyContent>
						</CnTab>
						<CnTab
							:title="`Zaken ${klant ? (zaken.length ? `(${zaken.length})` : '(0)') : ''}`">
						...
						<CnTab
							:title="`Taken ${klant ? (taken.length ? `(${taken.length})` : '(0)') : ''}`">
						...
						<CnTab
							...
							<div v-if="klant?.producten?.length">
								<NcListItem
									v-for="(product, key) in klant.producten"
									:key="key"
									:name="product.naam ?? 'N/A'"
									:bold="false"
									:details="product.omschrijving ?? 'N/A'"
		...
		<template #actions>
			...
			<NcButton
				...
				variant="primary"
				:disabled="
					!contactMomenten[selectedContactMoment]?.klant
					|| !medewerkers.values[selectedContactMoment - 1]?.id
					|| !channels.values[selectedContactMoment - 1]?.value
					|| loading
					|| success
					|| fetchLoading
				"
...
export default {
	 ...
	 */
	mounted() {
		const contactMomentId =
			this.contactMomentId ?? contactMomentStore.contactMomentItem?.id ?? null
	...
	methods: {
		...
		async fetchData(id, index) {
			...
			this.channels.values[index - 1] =
				this.channels.options.find(
					(channel) => channel.value === data.kanaal,
				) ?? null
				...
				.then(
					([
						...
						const selectedMedewerker =
							users.find(
								(medewerker) => medewerker.id === medewerkerToSelect,
							) || null
							...
							values: [
								...
								selectedMedewerker
									? {
											...
											displayName:
												selectedMedewerker.displayname,
				...
				.saveContactMoment(
					{
						...
						startDate:
							contactMomentCopy.startDate ?? new Date().toISOString(),
						status:
							contactMomentCopy.status === 'gesloten'
								? 'gesloten'
								: 'open',
						contactmoment: contactMomentCopy.selectedKlantContactMoment,
						medewerker:
							this.medewerkers.values[this.selectedContactMoment - 1]
								.id,
						kanaal: this.channels.values[this.selectedContactMoment - 1]
							.value,
					},
		...
		closeTaakForm(e) {
			if (e) {
				this.contactMomenten[this.selectedContactMoment]?.klant?.id
					&& this.fetchKlantData(
						this.contactMomenten[this.selectedContactMoment]?.klant?.id,
					).then(() => {
		...
		async closeContactMoment(id) {
			...
			contactMomentStore
				.saveContactMoment(newContactMoment, {
					redirect: !this.dashboardWidget,
				})
				.then(({ response }) => {
					if (response.ok) {
		...
		async fetchKlantData(klant) {
			...
			try {
				const klantResponse = await fetch(
				...
				if (this.isView) {
					...
					this.contactMomenten[this.selectedContactMoment].klant =
						await klantResponse.json()
				}
				...
				const results = await Promise.allSettled([
					fetch(
						`/index.php/apps/zaakafhandelapp/api/klanten/${klantId}/zaken`,
					),
					fetch(
						`/index.php/apps/zaakafhandelapp/api/klanten/${klantId}/taken`,
					),
					fetch(
						`/index.php/apps/zaakafhandelapp/api/klanten/${klantId}/berichten`,
					),
					fetch(
						`/index.php/apps/zaakafhandelapp/api/klanten/${klantId}/audit_trail`,
					),
					fetch(
						`/index.php/apps/zaakafhandelapp/api/klanten/${klantId}/contactmomenten`,
					),
				])
				...
				if (zakenData?.results && Array.isArray(zakenData.results)) {
					if (this.isView) {
						...
						this.contactMomenten[this.selectedContactMoment].zaken =
							zakenData.results
					}
				}
				...
				if (takenData?.results && Array.isArray(takenData.results)) {
					if (this.isView) {
						...
						this.contactMomenten[this.selectedContactMoment].taken =
							takenData.results
					}
				}
				...
				if (berichtenData?.results && Array.isArray(berichtenData.results)) {
					if (this.isView) {
						...
						this.contactMomenten[this.selectedContactMoment].berichten =
							berichtenData.results
					}
				}
				...
				if (auditTrailData && Array.isArray(auditTrailData)) {
					if (this.isView) {
						...
						this.contactMomenten[
							this.selectedContactMoment
						].auditTrails = auditTrailData
					}
				}

				if (
					contactMomentenData?.results
					&& Array.isArray(contactMomentenData.results)
				) {
					if (this.isView) {
						const filteredResults = contactMomentenData.results.filter(
							(contactMoment) =>
						...
						this.contactMomenten[
							this.selectedContactMoment
						].klantContactmomenten = contactMomentenData.results
					}
				}
			} catch (error) {
		...
		setSelectedZaak(id, zaak) {
			if (this.contactMomenten[id].selectedZaak === zaak) {
				...
				this.contactMomenten[id].selectedZaak = zaak
			}
		},
		/**
		 * @spec openspec/specs/ui-modals/spec.md#REQ-002
		...
		setSelectedTaak(id, taak) {
			if (this.contactMomenten[id].selectedTaak === taak) {
				...
				this.contactMomenten[id].selectedTaak = taak
			}
		},
		/**
		 * @spec openspec/specs/ui-modals/spec.md#REQ-002
		...
		setSelectedProduct(id, product) {
			if (this.contactMomenten[id].selectedProduct === product) {
				...
				this.contactMomenten[id].selectedProduct = product
			}
		},
	},
}
