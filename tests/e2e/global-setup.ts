 ...
 * from larpingapp's journeydoc setup.
 */
 ...
 *
 * The shared `Conduction/.github` Playwright job runs `npm ci` +
 * `npx playwright install` before the spec run, but never `npm run
 * build`. On a fresh CI VM the `js/zaakafhandelapp-main.js` artefact
