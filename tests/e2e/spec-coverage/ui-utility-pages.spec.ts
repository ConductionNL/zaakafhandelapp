...
test.describe('ui-utility-pages — dashboard, roadmap, audit-trail, settings', () => {
	...
	test('in-app dashboard — root route mounts the dashboard shell', async ({
		...
		await expect(navEntryByLabel(page, 'Dashboard')).toBeVisible({
			timeout: 10_000,
		})
	})
	...
	test('dashboard stats — the six manifest stats-block widgets render their titles', async ({
		...
		await expect(
			page.getByText('Open cases', { exact: false }).first(),
		).toBeVisible({ timeout: 10_000 })
		await expect(
			page.getByText('Open tasks', { exact: false }).first(),
		).toBeVisible()
		await expect(
			page.getByText('Persons', { exact: false }).first(),
		).toBeVisible()
		await expect(
			page.getByText('Organisations', { exact: false }).first(),
		).toBeVisible()
	})

	// @e2e openspec/specs/app-configuration/spec.md#features-and-roadmap
	test('features & roadmap — the roadmap page mounts (via in-app nav)', async ({
		page,
	}) => {
		await spaNavigate(page, '/features-roadmap')
		// The roadmap view renders its "Features" header + intro copy.
		await expect(
			page.getByRole('heading', { name: 'Features', exact: true }).first(),
		).toBeVisible({ timeout: 10_000 })
		await expect(
			page.getByText('Your input is the roadmap', { exact: false }).first(),
		).toBeVisible({ timeout: 10_000 })
	})
	...
	test('audit trail — the logs page mounts and renders rows or an empty state', async ({
		page,
	}) => {
		await spaNavigate(page, '/auditTrail')
		// The logs page host mounts.
		await expect(page.locator('[data-testid="cn-logs-page"]')).toBeVisible({
			timeout: 10_000,
		})
		...
		const hasEmpty = logsPage
			.getByText(/No log entries|no entries|empty/i)
			.first()
	...
	test('settings form — mounts on the Nextcloud admin settings page', async ({
		page,
	}) => {
		...
		await expect(page.locator('#zaakafhandelapp-settings')).toBeAttached({
			timeout: 15_000,
		})
		// The form renders its first section heading — proof the bundle
		// executed, not merely that the div exists.
		await expect(
			page.getByText('Data storage', { exact: false }).first(),
	...
	test('settings hard-nav — direct GET /settings returns 200 (BUG-3 fixed)', async ({
		request,
	}) => {
	...
	test('no uncaught JS exceptions on the working utility pages', async ({
		page,
	}) => {
