...
export async function useTableView(page: Page): Promise<void> {
	...
	if (await tableTab.first().isVisible({ timeout: 3_000 }).catch(() => false)) {
...
export async function openCreateModal(page: Page): Promise<Locator> {
	...
	await expect(dialog.getByRole('heading', { name: /^Create /i })).toBeVisible({ timeout: 8_000 })
	return dialog
}
 ...
 */
export async function fillField(scope: Locator, label: string, value: string): Promise<void> {
	const byRole = scope.getByRole('textbox', { name: new RegExp(`^${label}$`, 'i') }).first()
...
export async function submitModal(dialog: Locator): Promise<void> {
	const btn = dialog.getByRole('button', { name: 'Create' })
...
export async function rowFor(page: Page, index: Locator, term: string): Promise<Locator> {
...
export async function indexContains(page: Page, index: Locator, term: string): Promise<boolean> {
	await useTableView(page)
	const text = await index.locator('[data-testid="cn-object-list-table"]').textContent().catch(() => '')
	return (text || '').includes(term)
}

/** Open a row's actions menu and click one item (View/Edit/Copy/Delete). */
export async function rowAction(page: Page, row: Locator, action: 'View' | 'Edit' | 'Copy' | 'Delete'): Promise<void> {
	await row.locator('[data-testid="cn-row-actions"] button, [data-testid="cn-row-actions"]').first().click()
	await page.waitForTimeout(400)
	await page.getByRole('menuitem', { name: action, exact: true })
