...
module.exports = defineConfig([
	{
		...
		rules: {
			...
			// Prettier owns line layout, and this rule's verdict is a FUNCTION of
			// prettier's `printWidth`. It resolves to
			// `['error', 'always', { exceptAfterSingleLine: true }]`, so a class
			// member needs no blank line after it only while it fits on one line.
			// Prettier wraps the long union types in src/entities/*/**.ts —
			// `betrokkeneType`, `omschrijvingGeneriek` — across six lines each at
			// printWidth 85, the exception stops applying, and the rule reports 4
			// errors on code nobody edited.
			//
			// `eslint-config-prettier` does not cover it (verified:
			// `'lines-between-class-members' in require('eslint-config-prettier')`
			// is false) precisely because the conflict depends on a prettier
			// option rather than being unconditional. Turned off HERE, in a block
			// that runs before that config, so the "spread prettier last" rule
			// still holds — nothing after it can switch a stylistic rule back on.
	...
	// above is still present and still ON, because prettier has no opinion about
	// it. `indent` is now off HERE and enforced by prettier's `useTabs: true`
	// instead — the same tab, from the tool that also covers CSS and SCSS, which
